...
// FIX: Changed icon prop type to allow className to be passed via cloneElement.
...
const SubscriberRow: React.FC<SubscriberRowProps> = ({ subscription, user, isSelected, onToggleSelect }) => {
    ...
    return (
        <tr className="border-b border-gray-200 dark:border-gray-700/50">
            ...
            <td className="p-4 text-sm text-gray-900 dark:text-white font-mono whitespace-nowrap">
                {subscription.investedAmount.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}
            </td>
...
const CopyTradingManagementView: React.FC = () => {
    ...
    const isWeekend = new Date().getDay() === 0 || new Date().getDay() === 6; // Sunday is 0, Saturday is 6
    ...
    const confirmDistribution = () => {
        const percentage = parseFloat(profitPercentage);
        distributeCopyTradingProfits(selectedSubscriptionIds, percentage);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <StatCard title={t('investedAmount')} value={totalInvestedAmount.toLocaleString('en-US', { style: 'currency', currency: 'USD' })} icon={<CurrencyDollarIcon />} />
            </div>
            ...
            <Card>
                <div className="space-y-4 md:flex md:items-end md:space-y-0 md:space-x-4 rtl:space-x-reverse">
                    ...
                    <button
                        ...
                        disabled={selectedSubscriptionIds.length === 0 || !profitPercentage || isWeekend}
                        title={isWeekend ? "Profit distribution is disabled on weekends." : ""}
            ...
            <Modal isOpen={isConfirmModalOpen} onClose={() => setConfirmModalOpen(false)} title={t('confirmAction')}>
                 <div className="space-y-4">
                     <p>
                         {`Are you sure you want to distribute a ${profitPercentage}% profit to ${selectedSubscriptionIds.length} selected user(s)?`}
                    </p>
                    <p>
                        {`Total investment of selected users: ${totalSelectedInvestment.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`}
                    </p>
                     <p>
                        {`Total profit to be distributed: ${(totalSelectedInvestment * (parseFloat(profitPercentage) / 100)).toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`}
                    </p>
