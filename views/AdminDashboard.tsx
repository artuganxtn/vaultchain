...
const processSignupData = (users: User[], t: (key: string) => string) => {
    ...
    return last7Days.map(day => ({
        name: new Date(day).toLocaleDateString(t('en-US'), { weekday: 'short' }),
...
export const AdminDashboardView: React.FC = () => {
        ...
        const fetchKpis = async () => {
            try {
                ...
                if (data) {
                    ...
                    console.error('Failed to fetch KPIs: No data returned');
                    // Set default values to prevent infinite loading
                    setKpis({
                        totalUsers: 0,
                        totalPlatformBalance: 0,
                        totalMainBalances: 0,
                        totalInvestedBalances: 0,
                        totalOnHoldBalances: 0,
                        sessionsActiveNow: 0,
                        pendingDeposits: 0,
    ...
    const signupData = useMemo(() => processSignupData(users, t), [users, t]);
        ...
        <div className="space-y-8">
             ...
             <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <KPICard 
                        ...
                        value={kpis.totalPlatformBalance.toLocaleString('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 0 })}
                        gradient="bg-gradient-to-br from-blue-500 to-cyan-400"
                        breakdown={[
                            { label: t('totalMainBalances'), value: kpis.totalMainBalances.toLocaleString('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 0 }) },
                            { label: t('totalInvestedBalances'), value: kpis.totalInvestedBalances.toLocaleString('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 0 }) },
                            { label: t('totalOnHoldBalances'), value: kpis.totalOnHoldBalances.toLocaleString('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 0 }) },
                        ]}
                    />
                </div>
            </section>
            ...
            <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 bg-white dark:bg-[#062E1F]/80 backdrop-blur-sm border border-gray-200 dark:border-green-400/20 rounded-2xl p-5 shadow-md">
                    <h3 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white">Recent Signups (Last 7 Days)</h3>
                    <div className="h-80">
                    ...
                    <div className="space-y-4 max-h-96 overflow-y-auto">
                        {auditLogs.length > 0 ? auditLogs.slice(0, 50).map(log => (
                            <div key={log.id} className="text-sm border-b border-gray-200 dark:border-gray-700/50 pb-2 last:border-b-0">
                                ...
                                <p className="text-xs text-gray-400 dark:text-gray-500 text-end">{new Date(log.timestamp).toLocaleString()}</p>
                            </div>
                        )) : (
                            <p className="text-center text-gray-500 dark:text-gray-400 py-8">No admin activity recorded yet.</p>
                        )}
                    </div>
                </div>
            </section>
