...
const createTables = async () => {
    const createUserTable = `
        CREATE TABLE IF NOT EXISTS users (
            ...
            portfolio TEXT, passwordResetCode TEXT, passwordResetCodeExpires TEXT, investmentStartDate TEXT, isFeeExempt BOOLEAN, notification TEXT, profileImage TEXT,
            emailVerificationOTP TEXT, emailVerificationOTPExpires TEXT
        );`;
    ...
    // Migration: Add profileImage and OTP columns if they don't exist
    const columnsToAdd = [
        { name: 'profileImage', type: 'TEXT' },
        { name: 'emailVerificationOTP', type: 'TEXT' },
        { name: 'emailVerificationOTPExpires', type: 'TEXT' }
    ];
    ...
    for (const column of columnsToAdd) {
        try {
            ...
            if (!errMsg.includes('duplicate column') && !errMsg.includes('duplicate column name') && !errMsg.includes('already exists')) {
                console.log(`Note: Could not add ${column.name} column (may already exist):`, errMsg);
            } else {
                console.log(`${column.name} column already exists, skipping migration.`);
            }
        }
    }
};
