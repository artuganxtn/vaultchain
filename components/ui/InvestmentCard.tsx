...
const InvestmentCard: React.FC<InvestmentCardProps> = ({ user, todaysProfit, isCompact = false, isVerified, onVerifyClick, handleAddToInvestment, handleWithdrawProfit, handleRequestInvestmentWithdrawal, areProfitsWithdrawable }) => {
            ...
            <div className={`relative p-6 rounded-3xl overflow-hidden bg-gradient-to-br from-gray-800 to-black text-white shadow-2xl flex flex-col justify-between ${cardHeight}`}>
                ...
                <div className="flex justify-between items-start z-10">
                    ...
                    <div className="text-right rtl:text-left">
                        <p className="text-sm font-medium text-white/80 flex items-center justify-end space-x-2 rtl:space-x-reverse">{t('investedAmount')} {user.invested <= 0 && <span className="text-xs opacity-70">({t('unavailable')})</span>}</p>
                        <p className="text-3xl font-bold tracking-tight">
                            {user.invested.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}
                        </p>
                    </div>
                </div>
