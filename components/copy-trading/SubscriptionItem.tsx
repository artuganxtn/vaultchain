...
interface SubscriptionItemProps {
    ...
    // FIX: Changed return type to Promise<boolean> to match async function.
...
const SubscriptionItem: React.FC<SubscriptionItemProps> = ({ subscription, trader, onUnsubscribe, onUpdateSettings }) => {
    ...
    return (
        <>
            <div className="bg-white/60 dark:bg-gray-800/50 rounded-lg p-3 space-y-3">
                ...
                <div className="border-t border-gray-200 dark:border-gray-700/50 pt-2 flex justify-between items-center">
                    <div>
                         <p className="text-xs text-gray-500 dark:text-gray-400">{t('investedAmount')}</p>
                        <p className="font-medium text-gray-800 dark:text-gray-200">{investedAmount.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}</p>
                    </div>
                    <div className="flex items-center space-x-2 rtl:space-x-reverse">
                         <button onClick={() => setSettingsModalOpen(true)} className="px-3 py-2 text-xs font-semibold text-gray-700 dark:text-gray-200 bg-gray-200/80 dark:bg-gray-700/80 hover:bg-gray-300 dark:hover:bg-gray-600 rounded-lg transition-colors">
                            {t('settings')}
                        </button>
                        <button onClick={() => setConfirmModalOpen(true)} className="px-3 py-2 text-xs font-semibold text-white bg-red-600/90 hover:bg-red-600 rounded-lg transition-colors">
                            {t('unsubscribe')}
                        </button>
                    </div>
                </div>
