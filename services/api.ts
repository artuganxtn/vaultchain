...
// API Base URL - always uses relative path /api (works on same domain)
// This works for both development and production when deployed on vaultchaintr.com
...
// --- Data ---
export const getAllData = (): Promise<AppDataType> => fetch(`${API_BASE_URL}/data`).then(handleResponse);
...
export const getAiAssistanceStream = async (prompt: string, portfolioSummary: string, marketSummary: string): Promise<AsyncIterable<string>> => {
    ...
    if (!response.ok || !response.body) {
        throw new Error('Failed to get AI assistance stream');
    }
